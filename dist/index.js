...
document.addEventListener("DOMContentLoaded", function () {
    ...
    dropdown.addEventListener("change", () => {
        const cells = document.querySelectorAll(".priority");
    ...
    addRowBtn.addEventListener("click", function () {
        ...
        priorityCell.style.display =
            document.getElementById("algorithm").value === "pq"
                ? "block"
                : "none";
        // Add cells to row
    ...
    let Index;
    (function (Index) {
        Index[Index["ProcessId"] = 0] = "ProcessId";
        Index[Index["ArrivalTime"] = 1] = "ArrivalTime";
        Index[Index["BurstTime"] = 2] = "BurstTime";
        Index[Index["Priority"] = 3] = "Priority";
    })(Index || (Index = {}));
    calculateBtn.addEventListener("click", function () {
        if (tbody) {
            ...
            const rowData = rows.map((row) => {
                ...
                return cells.map((cell) => { var _a; return Number((_a = cell.textContent) === null || _a === void 0 ? void 0 : _a.trim()); });
            });
            const processes = rowData.map((row) => {
                return new Process(row[Index.ProcessId], row[Index.ArrivalTime], row[Index.BurstTime], row[Index.Priority]);
            });
            const dropdown = document.getElementById("algorithm");
...
function executeOutput(processes, algoType) {
    let scheduler;
    if (algoType === "fcfs") {
        ...
        scheduler = new RRScheduler(processes, Number(document.getElementById("timeQuantum").value));
    }
...
function renderVerticalGanttChart(processes, schedule) {
...
const timeCounter = document.querySelector(".count");
const arrow = document.querySelector(".arrow");
const readyQueueContainer = document.querySelector(".ready-queue .process");
const cpuContainer = document.querySelector(".cpu .process");
